...
config_file = "/home/raghuram/Prateek_Codes/Final On-Policy Experiments/MountainCar-Shaped/NAC_TD0/train_config.json"
...
load_A_C = config['value_critic']['load']
load_V_C = config['advantage_critic']['load']
    ...
    for iter in range(n_iters):
        ...
        for i in count():
            ...
            if done:
                ...
                print('Iteration: {}, Current Total Reward: {}, Running Total Reward: {}'.format(iter, i, round(running_total_reward,2)))
                ...
                actor.linears[0].weight.grad = -1*torch.reshape(torch.narrow(critic_weights, 1, 0, actor.all_layers_sizes[0]*actor.all_layers_sizes[1]), (actor.all_layers_sizes[1], actor.all_layers_sizes[0]))
                for j in range(1,len(actor.all_layers_sizes)-1):
                    actor.linears[j].weight.grad = -1*torch.reshape(torch.narrow(critic_weights, 1, actor.all_layers_sizes[j-1]*actor.all_layers_sizes[j], actor.all_layers_sizes[j]*actor.all_layers_sizes[j+1]), (actor.all_layers_sizes[j+1], actor.all_layers_sizes[j]))
                optimizerA.step()
                optimizerA.zero_grad()
